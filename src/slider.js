...
const slider = (() => {
  ...
  const currentSlideNumber = () => {
    const li = domQueries.ul()[0].children;
  ...
  const moveSlide = (oldIndex, newIndex) => {
    ...
    domQueries.liBullet(oldIndex).classList.remove('bg-light', 'border-dark');
    domQueries.liSlide(newIndex).classList.remove('d-none');
    domQueries.liSlide(newIndex).classList.add('d-block');
    domQueries.liBullet(newIndex).classList.remove('bg-dark', 'border-light');
  ...
  const displaySlider = () => {
    ...
    leftArrow.classList = 'arrows rounded-circle d-flex align-items-center justify-content-center arrow-left bg-dark border border-light text-white font-weight-bold position-relative float-left';
    const rightArrow = document.createElement('li');
    rightArrow.textContent = '>';
    rightArrow.classList = 'arrows rounded-circle d-flex align-items-center justify-content-center arrow-right bg-dark border border-light text-white font-weight-bold position-relative float-right';

    pictures.forEach((url, i) => {
      const slide = document.createElement('li');
      slide.style.backgroundImage = `url(${url})`;
      slide.setAttribute('id', `slide${i}`);
      slide.classList = `slide slide${i} ${i === 0 ? 'd-block' : 'd-none'} w-100 vh-100 rounded`;
